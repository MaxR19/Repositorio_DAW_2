...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($accion) {
        case 'guardar':
            modificarTarea($titulo, $estado, $id);
            volver();
            break;
        case 'eliminar':
            eliminarTarea($id);
...
function anadirTarea($titulo, $estado)
{
    ...
    $nuevoTitulo = (object)['id' => $id, 'titulo' => $titulo, 'estado' => $estado];

    // Añadimos la nueva tarea al array
    $coleccion[] = $nuevoTitulo;
...
function modificarTarea($nuevoTitulo, $nuevoEstado, $id)
{
    // Cargo el JSON en memoria
    $coleccion = cargarJSON('tareas.json');
    if ($coleccion === null) {
    ...
    foreach ($coleccion as &$comic) {
        if ($comic->id == $id) {
            $comic->titulo = $nuevoTitulo;  // cambia el texto de la tarea
